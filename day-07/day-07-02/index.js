...
const main = () => {
  ...
  const usedSpace = getFolderSize(folders[0]); //Calculate all the folders size
  const diskSpace = 70000000;
  const updateSpace = 30000000;
  const unsetSpace = diskSpace - usedSpace;
  const spaceNeeded = updateSpace - unsetSpace;

  // Filter folders that have at least the size that we need
  // Then map to only numbers
  // Sort asc
  // Get the first
  console.info(folders.filter((x) => x.size >= spaceNeeded).map((x) => x.size).sort((a, b) => a-b)[0]);
}
main();